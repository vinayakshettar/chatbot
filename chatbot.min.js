...
(function (window) {
    ...
    const DEFAULT_CONFIG = {
        ...
        endpoints: {
            ...
            uploadImage: '/api/tickets/upload',
    ...
    class SupportChatbot {
        ...
        .input-actions button { padding: 8px; background: #f3f4f6; border: none; border-radius: 6px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
        .input-actions button:hover { background: #e5e7eb; }
        .input-actions svg { width: 20px; height: 20px; fill: #6b7280; }
        ...
        .send-button { padding: 10px 16px; background: ${this.config.primaryColor}; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; font-family: inherit; }
        ...
        .submit-button { width: 100%; padding: 12px; background: #10b981; color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; margin-top: 8px; font-family: inherit; display: flex; align-items: center; justify-content: center; gap: 8px; }
        ...
        .image-preview-item { width: 60px; height: 60px; border-radius: 4px; overflow: hidden; position: relative; border: 2px solid #e5e7eb; }
        .image-preview-item img { width: 100%; height: 100%; object-fit: cover; }
        .image-preview-remove { position: absolute; top: -6px; right: -6px; background: #ef4444; color: white; border: none; border-radius: 50%; width: 20px; height: 20px; cursor: pointer; font-size: 12px; line-height: 1; }
        ...
        <div class="chatbot-window open">
          ...
          <div class="chatbot-input-area">
            ...
            <div class="input-actions">
              ...
              <button onclick="document.getElementById('chatbot-file-input').click()" title="Upload image">
                <svg viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4m14-7l-5-5-5 5m5-5v12"/></svg>
              </button>
            </div>
            <div class="input-row">
      ...
      `).join('');
        ...
        renderOptions() {
          ...
          ${this.options.impact.map(opt =>
                    `<button class="option-button" onclick="window.supportChatbot.selectOption('impact', '${opt.label}')">${opt.label}</button>`
                ).join('')}
        </div>`;
            }
          ...
          ${this.options.urgency.map(opt =>
                    `<button class="option-button" onclick="window.supportChatbot.selectOption('urgency', '${opt.label}')">${opt.label}</button>`
                ).join('')}
        </div>`;
            }
          ...
          ${this.options.priority.map(opt =>
                    `<button class="option-button" onclick="window.supportChatbot.selectOption('priority', '${opt.label}')">${opt.label}</button>`
                ).join('')}
        </div>`;
            }
            ...
            try {
                ...
                if (this.config.endpoints.intentDetection) {
                    ...
                    if (lowerMessage.includes('error') || lowerMessage.includes('issue') ||
                        ...
                        lowerMessage.includes('help') || lowerMessage.includes('not working')) {
                        this.state.currentStep = 'impact';
                        this.addMessage('bot', "I understand you're experiencing an issue. Let me help you create a ticket. First, who is affected?");
                    } else {
                        this.addMessage('bot', "I can help you report issues. Would you like to report a problem?");
        ...
        handleFileUpload(event) {
            ...
            files.forEach(file => {
                ...
                if (file.size > this.config.maxFileSize) {
                    this.addMessage('bot', `❌ File too large: ${file.name} (max ${this.config.maxFileSize / 1024 / 1024}MB)`);
                    return;
                }
        ...
        async submitTicket() {
            ...
            try {
                ...
                const response = await this.apiCall(
                    this.config.endpoints.createTicket,
                    ticketPayload
                );
