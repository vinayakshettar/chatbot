...
(function (window) {
    ...
    const DEFAULT_CONFIG = {
        // Your .NET Core API endpoints
        apiBaseUrl: 'http://localhost:5000',
        endpoints: {
            createTicket: '/api/ticket',
            intentDetection: '/api/chatbot/intent' // Optional
        },
        ...
        // Custom headers for API calls (for authentication)
        headers: {
        ...
        // Callbacks
        onTicketCreated: null, // function(ticketId, ticketData) {}
    ...
    class SupportChatbot {
        ...
        getConversationContext() {
            ...
            return {
                ...
                detectedErrors: this.state.detectedErrors,
                messageHistory: this.state.messages.map(m => ({
                    role: m.sender,
                    content: m.text,
        ...
        setupErrorDetection() {
            ...
            window.XMLHttpRequest = function () {
                ...
                xhr.addEventListener('error', () => {
                    window.supportChatbot.captureError({
                        type: 'XHR Error',
        ...
        captureError(error) {
            ...
            if (error.type.includes('500') || error.type.includes('Cannot') ||
        ...
        .chatbot-window { width: 380px; height: 600px; background: white; border-radius: 12px; box-shadow: 0 8px 24px rgba(0,0,0,0.2); display: none; flex-direction: column; overflow: hidden; }
        ...
        .input-actions .action-btn:active { transform: scale(0.95); }
        ...
        .image-preview-remove { position: absolute; top: -8px; right: -8px; background: #ef4444; color: white; border: none; border-radius: 50%; width: 24px; height: 24px; cursor: pointer; font-size: 14px; line-height: 1; display: flex; align-items: center; justify-content: center; transition: all 0.2s; }
        .image-preview-remove:hover { background: #dc2626; transform: scale(1.1); }
        .screenshot-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 999998; display: none; }
        .screenshot-overlay.active { display: flex; align-items: center; justify-content: center; }
      ...
      `;
        ...
        renderChatbot() {
            return `
        ${!this.state.isOpen ? this.renderButton() : this.renderWindow()}
      `;
        }
        ...
        <div class="chatbot-window open">
          ...
          <div class="chatbot-input-area">
            ...
            <div class="input-actions">
              ...
              <button id="file-upload-btn" onclick="document.getElementById('chatbot-file-input').click()" title="Upload image" class="action-btn">
                <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
              </button>
              ${this.config.enableScreenshot ? `<button id="screenshot-btn" onclick="window.supportChatbot.captureScreenshot()" title="Take screenshot" class="action-btn">
                <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>
              </button>` : ''}
            </div>
            <div class="input-row">
      ...
      `).join('');
        ...
        renderOptions() {
          ...
          ${this.options.impact.map(opt =>
                    `<button class="option-button" onclick="window.supportChatbot.selectOption('impact', '${opt.id}')">${opt.label}</button>`
                ).join('')}
        </div>`;
            }
            ...
            if (step === 'urgency' && !data.urgency) {
                return `<div class="options-container">
          ${this.options.urgency.map(opt =>
                    `<button class="option-button" onclick="window.supportChatbot.selectOption('urgency', '${opt.id}')">${opt.label}</button>`
                ).join('')}
        </div>`;
            }
          ...
          ${this.options.priority.map(opt =>
                    `<button class="option-button" onclick="window.supportChatbot.selectOption('priority', '${opt.id}')">${opt.label}</button>`
                ).join('')}
        </div>`;
            }
          ...
          ${this.options.category.map(cat =>
                    `<button class="option-button" onclick="window.supportChatbot.selectOption('category', '${cat}')">${cat}</button>`
                ).join('')}
        </div>`;
            }
      ...
      `;
        ...
        send() {
            ...
            } else if (this.state.currentStep === 'description') {
                this.state.ticketData.description = message;
                this.state.currentStep = 'contact';
                this.addMessage('bot', 'Please provide your email address or phone number:');
        ...
        async detectIntent(message) {
            ...
            try {
                // Try API intent detection if endpoint is configured
                if (this.config.endpoints.intentDetection) {
                    const response = await this.apiCall(this.config.endpoints.intentDetection, {
                        message: message,
                        sessionId: this.state.sessionId,
                        context: this.getConversationContext()
                    });

                    if (response.intent === 'report_issue') {
                        this.state.currentStep = 'impact';
                        this.addMessage('bot', "I understand you're experiencing an issue. Let me help you create a ticket. First, who is affected?");
                    } else if (response.intent === 'get_help') {
                        this.addMessage('bot', "I can help you report issues. Would you like to report a problem?");
                    } else {
                        // For unrecognized intents, ask clarifying question
                        this.addMessage('bot', "I'm here to help with issue reporting. Are you experiencing a problem you'd like to report?");
                    }
                } else {
                    ...
                    if (lowerMessage.includes('error') || lowerMessage.includes('issue') ||
                        ...
                        // For other messages, provide a gentle prompt
                        this.addMessage('bot', "I'm here to help with issue reporting. Are you experiencing a problem you'd like to report?");
                    }
                }
            } catch (error) {
        ...
        selectOption(step, value) {
            ...
            const nextSteps = {
                category: { next: 'description', msg: 'Please describe the issue in detail:' },
        ...
        handleFileUpload(event) {
            ...
            if (totalFiles > this.config.maxFiles) {
                this.addMessage('bot', `❌ Too many files. Maximum ${this.config.maxFiles} files allowed. Currently have ${this.state.uploadedImages.length}, trying to add ${files.length}.`);
                event.target.value = '';
                return;
            }
            ...
            files.forEach(file => {
                ...
                if (!this.config.allowedFileTypes.includes(file.type)) {
                    this.addMessage('bot', `❌ File type not allowed: ${file.name} (supported: ${this.config.allowedFileTypes.map(t => t.split('/')[1]).join(', ')})`);
                    return;
                }
                ...
                if (file.size > this.config.maxFileSize) {
                    ...
                    this.addMessage('bot', `❌ File too large: ${file.name} (${fileMB}MB, max ${maxMB}MB)`);
                    return;
                }
                ...
                reader.onload = (e) => {
                    ...
                    successCount++;
                    this.addMessage('bot', `✅ Image uploaded: ${file.name} (${(file.size / 1024).toFixed(1)}KB)`);
                    this.render();
                };

                reader.onerror = (error) => {
                    console.error('File read error:', error);
                    this.addMessage('bot', `❌ Error reading file: ${file.name}`);
                };
        ...
        async captureScreenshot() {
            ...
            const btn = this.container?.querySelector('#screenshot-btn');
            if (btn) {
                btn.disabled = true;
            ...
            try {
                ...
                if (btn) {
                    btn.disabled = false;
                    btn.innerHTML = '<svg viewBox="0 0 24 24"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>';
                }
            }
        }
        ...
        async submitTicket() {
            ...
            try {
                const ticketPayload = {
                    ...
                    errors: this.state.detectedErrors
                };
                ...
                this.state.ticketData.images?.forEach((file, index) => {
                    formData.append("files", file); // or "images"
                });
                ...
                const ticketId = response.ticketId || response.id || 'TK-' + Date.now();

                this.addMessage('bot', `✅ Success! Your ticket #${ticketId} has been created. Our team will review it soon!`);

                // Callback
                if (this.config.onTicketCreated) {
                    this.config.onTicketCreated(ticketId, ticketPayload);
                }
                ...
                this.addMessage('bot', '❌ Sorry, there was an error creating your ticket. Please try again or contact support directly.');
